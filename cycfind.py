...
def binary_search(lis, query):
    ...
    :param lis: This parameter must be a list of tuples, as following [(key,val),...].
    :param query:
    :return: Will be a String type value, if there is no data in list that satisfies the query,
    ...
    if lis[low][0] == 0:
        ...
        # main statements of binary search
        while high >= low:
            ...
            if lis[mid][0] == query:
                ...
                # decision which part of list to choose
                if lis[mid][0] < query:
                    low = mid + 1
                else:
                    high = mid - 1
...
def cyclic_search(lis, query):
    ...
    :param query:
    :return: Will be a String type value, if there is no data found NameError Eception will be raised.
    """
    low = 0
    high = len(lis)-1
    result = ""
    ...
    if lis[low][0] == query:
        ...
        while low <= high:
            ...
            if lis[mid][0] == query:
                ...
                if query < lis[mid][0]:
                    # This holds when the query must be in sorted part of the list (a.k.a in LHS part).
                    ...
                    # Means that the query is in RHS which is unsorted so the focus is switched to that part.
                    low = mid + 1
            elif lis[mid][0] < lis[high][0]:
                # This case holds when the right hand side of the list is sorted.
                if query > lis[mid][0]:
                    # This holds when the query must be in sorted part of the list (a.k.a in RHS part).
                    ...
                    # Means that the query is in LHS which is unsorted so the focus is switched to that part.
                    high = mid - 1
    if result == "":
